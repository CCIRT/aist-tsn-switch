# Design of 10GbE ats-switch

This document describes design of the 10GbE, 4-port L2 switch with ATS implemented on FPGA.

## Overview

![overview](./img/overwiew_ats-switch-10g.drawio.svg)
...
![mac_block](../cbs-switch/img/mac_block_10g.drawio.svg)
...
- The timer is managed as unsigned integer 72 bit
  - The value is zero when the power is turned on, and is incremented by 6,400 every 1 cycle
...
- The set value is the value of Ref. Timer block when the first beat of the Ethernet frame passed
  - i.e. when both tvalid and tready are asserted for the first time after tlast is asserted at the AXI4-Stream input
...
- Input frame is a frame with Arrival Time.
- Eligibility Time calculation is done in ATS block.

---

- ATS block controls output of Ethernet frame based on the comparison result of Eligibility Time and Ref. Timer
...
|               | Supported TCs                           | **TC0-TC7**               | **TC0-TC7**               | **TC5-TC7** (\*1)        |
|               | TC Scheduling (TC6, TC7)                | ATS                       | ATS                       | ATS                      |
|               | TC Scheduling (Others)                  | Strict Priority           | Strict Priority           | Strict Priority          |
| Internal FIFO |                                         |                           |                           |                          |
|               | MAC block (\*2)                         | 4096 bytes (default)      | 4096 bytes (default)      | 4096 bytes (default)     |
|               | FIFO block (\*2)                        | **- (\*4)**               | **4096 bytes (default)**  | **4096 bytes (default)** |
|               | ATS Preprocess block (\*2)              | **0 bytes (default)**     | **0 bytes (default)**     | **2048 bytes (packet)**  |
|               | Switch with FDB block (\*2)             | **3072 bytes (default)**  | **3072 bytes (default)**  | **2048 bytes (default)** |
|               | ATS block TC7 (\*2)                     | **6144 bytes (packet)**   | **6144 bytes (packet)**   | **4096 bytes (packet)**  |
|               | ATS block TC6 (\*2)                     | **6144 bytes (packet)**   | **6144 bytes (packet)**   | **4096 bytes (packet)**  |
|               | ATS block TC5 (\*2)                     | 8192 bytes (default)      | 8192 bytes (default)      | 8192 bytes (default)     |
|               | ATS block TC4 (\*2)                     | **8192 bytes (default)**  | **8192 bytes (default)**  | **0 bytes (default)**    |
|               | ATS block TC3 (\*2)                     | **8192 bytes (default)**  | **8192 bytes (default)**  | **0 bytes (default)**    |
|               | ATS block TC2 (\*2)                     | **16384 bytes (default)** | **16384 bytes (default)** | **0 bytes (default)**    |
|               | ATS block TC1 (\*2)                     | **16384 bytes (default)** | **16384 bytes (default)** | **0 bytes (default)**    |
|               | ATS block TC0 (\*2)                     | **16384 bytes (default)** | **16384 bytes (default)** | **0 bytes (default)**    |
| ATS logic     |                                         |                           |                           |                          |
|               | Sorter between input ports              | **ET order**              | **ET order**              | **Round-Robin**          |
|               | Accuracy of Eligibility Timestamp (\*3) | **More accurate**         | **More accurate**         | **Less accurate**        |
| FDB           |                                         |                           |                           |                          |
...
- \*4: This FIFO is not necessary for operation and has been omitted to reduce latency.
