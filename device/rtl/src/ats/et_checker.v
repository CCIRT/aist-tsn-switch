...
module et_checker #(
  ...
  output reg [TIMESTAMP_WIDTH-1:0] m_axis_eligibility_timestamp_tdata,
  output reg                       m_axis_eligibility_timestamp_tvalid,
  ...
  localparam STATE_PASS_TIMESTAMP = 2;
  // Important registers
  ...
  always @ (posedge clk) begin
    if (!rstn) begin
      ...
      m_axis_eligibility_timestamp_tdata <= 'd0;
      m_axis_eligibility_timestamp_tvalid <= 'd0;
    end else begin
      case (state)
        ...
        STATE_COMPARE_TIMESTAMP_WITH_TIMER: begin
          if (assigned_eligibility_timestamp_reg < transmission_selection_timer_with_delay) begin
            state <= STATE_PASS_TIMESTAMP;
            m_axis_eligibility_timestamp_tdata <= assigned_eligibility_timestamp_reg;
            m_axis_eligibility_timestamp_tvalid <= 1'b1;
          end else begin
            // Do nothing
          end
        end
        STATE_PASS_TIMESTAMP: begin
          if (m_axis_eligibility_timestamp_tvalid & m_axis_eligibility_timestamp_tready) begin
            state <= STATE_WAIT_FOR_TIMESTAMP;
            m_axis_eligibility_timestamp_tvalid <= 1'b0;
          end else begin
            // Do nothing
          end
        end
