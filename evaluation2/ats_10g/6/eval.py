...
class TestModule(utility.TestModuleBase):
    ...
    def test(self, tc5_rate, tc7_rate, num_frames, result_dir):
        if self.args.efcc_type == '1g':
            ...
            self._prepare_ats_param(self.send_port0, 7, 10000, COMMITTED_BURST_SIZE)
        L3L4_HEADER_SIZE = 28
        _, sz7 = self._prepare_frames_with_rate(self.send_port0, self.recv_port, 7, tc7_rate, header_size=L3L4_HEADER_SIZE)
        # _, sz5 = self._prepare_burst_frames_with_rate(self.send_port1, self.recv_port, 5, tc5_rate, header_size=L3L4_HEADER_SIZE)
        _, sz5 = self._prepare_frames_with_rate(self.send_port1, self.recv_port, 5, tc5_rate, header_size=L3L4_HEADER_SIZE)

        if tc5_rate == 0:
            send_ports = [self.send_port0]
        else:
...
def main():
    print('Evaluation 6: Measure the latency of an ATS flow competing with a best-effort flow.')
    ...
    parser.add_argument('--tc5_rates', default=[0, 1000, 3000, 5000, 7000, 9000, 10000], type=int, nargs='+', help='test tc7 rates')
    ...
    for tc5_rate in args.tc5_rates:
        if args.efcc_type == '1g':
            ...
            tc7_rate = 997
        num_frames = int(2000 * (tc7_rate + tc5_rate) / tc7_rate)
        print(f'tc7_rate = {tc7_rate} Mbps, tc5_rate = {tc5_rate} Mbps, num_frames = {num_frames}')
        tm.test(tc5_rate, tc7_rate, num_frames, f'results/tc7_{tc7_rate}_tc5_{tc5_rate}')
