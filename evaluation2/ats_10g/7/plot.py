...
from ats_default_params import MAX_RESIDENCE_TIME
...
max_residence_times = [MAX_RESIDENCE_TIME]
cirs = [0, 1000, 3000, 5000, 7000, 9000, 10000]
...
mrt = MAX_RESIDENCE_TIME
for cir in cirs:
    print('------------------------------')
    print(f'cir = {cir}, port0 (TC7)')
    dirname = f'mrt_{mrt}_tc7_CIR{cir}'
    timestamp_lats = TimestampSummary(f'results/{dirname}/diff_port0.csv')
    plot_latency_points(timestamp_lats, 156.25, f'results/each_frame_{cir}.png')

    if cir >= 9000:
        print(f'cir = {cir}, port0 (TC7), skip first 100 frames')
        plot_latency_points(timestamp_lats, 156.25, f'results/each_frame_{cir}_skip100.png', y_start=100)

    if cir != 0:
        print(f'cir = {cir}, port1 (TC5)')
        dirname = f'mrt_{mrt}_tc7_CIR{cir}'
        timestamp_lats = TimestampSummary(f'results/{dirname}/diff_port1.csv')
        plot_latency_points(timestamp_lats, 156.25, f'results/each_frame_{cir}_mrt{mrt}_port1.png')
