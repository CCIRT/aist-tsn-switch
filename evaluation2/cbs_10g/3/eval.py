...
class TestModule(utility.TestModuleBase):
    ...
    def _prepare_frames(self, num_frames, tc, average_rate_mbps, stddev):
        ...
        valid_send_size = length + HEADER_SIZE + FRAME_INTERVAL
        valid_send_time = valid_send_size / BYTES_PER_CYCLE
        total_send_time = math.ceil(valid_send_size * BITS_PER_BYTE / NS_PER_CYCLE / average_rate_mbps * 1000)
        ...
        return valid_frame.get_length()[::-1]
    ...
    def test(self, num_frames, avg_rate, stddev, result_dir):
        ...
        if self.args.efcc_type == '1g':
            ...
            # slope (TC7): idle =  100000 Kbps, send = -9900000 Kbps
            send_slope = idle_slope - 10000000
        print(f'idle_slope = {idle_slope}, send_slope = {send_slope}')
        self.cbs.set_slope(tc, idle_slope, send_slope)

        _, frame_size = self._prepare_frames(num_frames, tc, avg_rate, stddev)
        ...
        self._report_bandwidth(results, [self.send_port], self.recv_port, lambda x: frame_size, result_dir)
...
def main():
    ...
    parser.add_argument('--avg_rate_mbps', default=2500, type=int, help='the average network rate')
    parser.add_argument('--stddev_cycles', default=125, type=int, help='the standard deviation of frame interval (cycles)')
