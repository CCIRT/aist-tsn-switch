...
def common_argument(name: str):
    ...
    parser.add_argument('--device-frequency', type=float, default=156.25, help='Device frequency (MHz) of TSN EFCC')
    ...
    parser.add_argument('--efcc_type', default='10g', type=str, help='1g or 10g')
...
class TestModuleBase():
    def __init__(self, args):
        ...
        if self.args.efcc_type == '1g':
            addr_table = tsn_efcc.default_tsn_efcc_address_table()
            config = tsn_efcc.default_axis_switch_config_table()
        ...
        elif self.args.efcc_type == '10g':
            addr_table = tsn_efcc.default_tsn_efcc_address_table_10g()
            config = tsn_efcc.default_axis_switch_config_table_10g()
        ...
        self.switch = tsn_efcc.AxisNetSwitch(self.xsdb, self.args.efcc_jtag_target, addr_table, config)
        self.switch.start_config()
        self.switch.connect(config.num_valid_ports, 0)
        self.switch.connect(config.num_valid_ports + 1, 1)
        self.switch.connect(config.num_valid_ports + 2, 2)
        self.switch.connect(config.num_valid_ports + 3, 3)
        self.switch.commit()
    ...
    def _prepare_frames_with_rate_await(self, send_port, recv_port, tc, rate_mhz, header_size=None, no_magic=False, random_frames=1000):
        """Prepare frames for constant input rate.
        """
        dst_mac, _ = get_default_mac(self.crafter, recv_port)
        src_mac, _ = get_default_mac(self.crafter, send_port)
        ...
        for i in range(random_frames):
            # self.crafter.set_frame(send_port, idx, frame)

            if nop_length > default_nop_length:
                random_number = random.randrange(128)
                frame2a = frame.clone().additional_wait(random_number)
                self.crafter.set_frame(send_port, idx, frame2a)

                index = idx + 1
                frame2 = frame.clone().payload(default_nop_payload).nop()
                while nop_length > default_nop_length:
                    self.crafter.set_frame(send_port, index, frame2)
                    nop_length -= default_nop_length
                    index += 1

                # append payload to last frame
                remain = nop_length
                frame2.payload(default_nop_payload + remain)
                self.crafter.set_frame(send_port, index - 1, frame2)
                idx = index
            elif nop_length < 0:
                # send frame without additional interval
                ...
                # Since nop frame length become minus, use additonal wait instead.
                frame2 = frame.clone().additional_wait(nop_length)
            ...
            if True:
            # if show_progress:
                self._show_progress_bar('prepare_frames', i, random_frames)

        self.crafter.set_frame(send_port, idx, eol_frame)
        idx += 1
        if True:
        # if show_progress:
            self._show_progress_bar('prepare_frames', random_frames, random_frames)
            print('')

        print(f'TC7: {idx+1} frames are generated.')
        self._show_whole_test_sequence(send_port)
    ...
    def _do_transfer(self, crafter_ports, capture_ports, num_frames, initial_wait=0):
