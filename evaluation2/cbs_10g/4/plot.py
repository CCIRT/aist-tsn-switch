...
def plot_frame_loss(records, title, filename):
    ...
    plt.legend()
    plt.title(title)
    plt.xlabel('Transmit rate of TC5 (Port 1) [Mbps]')
...
rates0 = [0, 2000, 4000, 6000, 8000, 10000]
rate1 = 10000
...
records = defaultdict(lambda: [BandwidthSummary(), BandwidthSummary()])
for rate0 in rates0:
    dirname = f'tc{tc0}_{rate0}_tc{tc1}_{rate1}_with_cbs'
    if rate0 != 0:
        records[rate0][0].read_file(f'results/{dirname}/port0_summary_bandwidth.txt')
    records[rate0][1].read_file(f'results/{dirname}/port1_summary_bandwidth.txt')

plot_frame_loss(records, f'Frame Loss (Port 0: TC{tc0} + CBS, Port 1: TC{tc1})', f'results/frame_loss_tc{tc0}_tc{tc1}_with_cbs.png')
plot_arrival_rate(records, f'Arrival Rate (Port 0: TC{tc0} + CBS, Port 1: TC{tc1})', f'results/arrival_rate_tc{tc0}_tc{tc1}_with_cbs.png')
