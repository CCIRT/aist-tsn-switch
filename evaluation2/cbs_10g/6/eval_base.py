...
class TestModule(utility.TestModuleBase):
    def __init__(self, args):
        ...
        self.cbs.set_traffic_class(DEFAULT_PRIO, 5)
        ...
        if self.args.efcc_type == '1g':
            ...
            idle_slope =   1000 * 1000
            send_slope = -10000 * 1000 + idle_slope
        self.cbs.set_slope(6, idle_slope, send_slope)
        self.cbs.set_slope(7, idle_slope, send_slope)

    def test(self, tc5_rate, tc6_rate, tc7_rate, num_frames, result_dir, initial_wait=0, random_await=False, random_frames=1000):
        ...
        print(f'send_port0 (TC5) = {self.send_port0}, send_port1 (TC6) = {self.send_port1}, send_port2 (TC7) = {self.send_port2}')
        enabled_ports = []
        frame_size = {}
        ...
        if tc7_rate != 0:
            ...
            if random_await:
                print('Randomize TC7 send rate')
                _, sz = self._prepare_frames_with_rate_await(port, self.recv_port, 7, tc7_rate, random_frames=random_frames)
            else:
                _, sz = self._prepare_frames_with_rate(port, self.recv_port, 7, tc7_rate)
            frame_size[port] = sz
            enabled_ports.append(port)

        results = self._do_transfer(enabled_ports, [*enabled_ports, self.recv_port], num_frames, initial_wait)
